...
export default function LoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      // 🔥 REN SUPABASE LOGIN (ingen redirectTo)
      ...
      console.error("Google login crash:", e);
      setError("Google login misslyckades");
      setLoading(false);
    }
  };
  ...
  return (
    <div
      className="min-h-[100dvh] flex items-center justify-center p-6 bg-background"
      style={{
        background: "radial-gradient(circle at 20% 0%, rgba(31,111,84,0.25), transparent 40%), #0B1220",
      }}
    >
      <div
      ...
      >
        <div className="flex flex-col items-center mb-6">
          ...
          <h1 className="text-lg font-semibold text-white">CLC Fastighetsservice</h1>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-3">
          ...
          <button type="submit" disabled={loading} className="w-full py-3 rounded-xl bg-primary text-white font-medium">
            {loading ? <Loader2 className="animate-spin mx-auto" size={18} /> : "Logga in"}
          </button>
        </form>
        ...
        <p className="text-[11px] text-center text-muted-foreground mt-5">Endast behöriga användare</p>
      </div>
    </div>
  );
}
